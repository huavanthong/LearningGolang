...
type MongoField2 struct {
	FieldStr  string `json: "Field Str"`
	FieldDesc string `json: "Field Desc"`
	FieldInt  int    `json: "Field Int"`
	FieldBool bool   `json: "Field Bool"`
}
...
func main() {
	...
	twoDoc := MongoField2{
		FieldStr:  "This is our first data and its very important",
		FieldDesc: "This is a description",
		FieldInt:  826482746,
		FieldBool: true,
	}

	fmt.Println("twoDoc Type: ", reflect.TypeOf(twoDoc), "\n")

	result2, insertErr := col.InsertOne(ctx, twoDoc)
	if insertErr != nil {
		fmt.Println("InsertONE Error:", insertErr)
		os.Exit(1)
	} else {
		fmt.Println("InsertOne() result type: ", reflect.TypeOf(result2))
		fmt.Println("InsertOne() api result type: ", result2)

		newID := result2.InsertedID
		fmt.Println("InsertedOne(), newID", newID)
		fmt.Println("InsertedOne(), newID type:", reflect.TypeOf(newID))
