/***********
In this tutorial, you will see how to use the Find and FindOne functions to filter for documents
within a collection.
...
func main() {
	...
	id, _ := primitive.ObjectIDFromHex("631063ce4ccfecd6152975d0")
	...
	lookupStage2 := bson.D{{"$lookup", bson.D{{"from", "podcasts"}, {"localField", "podcast"}, {"foreignField", "_id"}, {"as", "podcast"}}}}
	unwindStage2 := bson.D{{"$unwind", bson.D{{"path", "$podcast"}, {"preserveNullAndEmptyArrays", false}}}}

	showLoadedStructCursor, err := episodesCollection.Aggregate(ctx, mongo.Pipeline{lookupStage2, unwindStage2})
	if err != nil {
		panic(err)
	}
